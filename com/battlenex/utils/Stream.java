...
public class Stream {
	...
	public void writeByteA(int i, int i2) {
		ensureCapacity(1);
		buffer[currentOffset++] = (byte) (i + 128);
		buffer[currentOffset++] = (byte) (i2 + 128);
	...
	public void writeWord(int i, int i2) {
		...
		buffer[currentOffset++] = (byte) (i >> 8);
		buffer[currentOffset++] = (byte) i;
		buffer[currentOffset++] = (byte) (i2 >> 8);
	...
	public void writePacketSizeWord(int i) {
		buffer[bufferLocation - i - 2] = (byte) (i >> 8);
		buffer[bufferLocation - i - 1] = (byte) i;
	}
	
	public void writePacketSizeByte(int i) {
		buffer[bufferLocation - i - 1] = (byte) i;
	}

	public void endPacketReservedByte() {
		if (frameStackPtr < 0) {
			throw new RuntimeException("Stack empty");
		} else {
			writePacketSizeByte(bufferLocation - frameStack[frameStackPtr--]);
		}
	}
	...
	public void endPacketReservedWord() {
		if (frameStackPtr < 0) {
			...
			writePacketSizeWord(bufferLocation - frameStack[frameStackPtr--]);
		}
	}
	
	public void writeWordReversed(int i) {
		ensureCapacity(2);
		buffer[bufferLocation++] = (byte) i;
		buffer[bufferLocation++] = (byte) (i >> 8);
	}
	
	public void createFrameReservedWord(int id) {
		ensureCapacity(2);
		buffer[bufferLocation++] = (byte) (id + encryption.getNextKey());
		writeWord(0);
		if (frameStackPtr >= frameStackSize - 1) {
			throw new RuntimeException("Stack overflow");
		} else {
			frameStack[++frameStackPtr] = bufferLocation;
		}
	}

	public void createFrameReservedByte(int id) {
		ensureCapacity(3);
		buffer[bufferLocation++] = (byte) (id + encryption.getNextKey());
