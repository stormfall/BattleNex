...
import com.battlenex.world.WorldTile;
...
public class NPC {
	...
	public NPC(int _npcId, WorldTile worldTile, int _npcType, boolean b, boolean c) {
		npcId = _npcId;
		npcType = _npcType;
		direction = -1;
		isDead = false;
	...
	public int getNextWalkingDirection() {
		...
		if(!WalkingCheck.tiles.containsKey(currentTile.getH() << 28 | currentTile.getX() << 14 | currentTile.getY())){
			int dir;
			dir = Utils.direction(absX, absY, (absX + moveX), (absY + moveY));
			if (dir == -1)
				return -1;
			dir >>= 1;
			absX += moveX;
