...
public class UseItem {
	...
	public static void ItemonObject(Client c, int objectID, int objectX,
		...
		default:
			if (c.playerRights == 3)
				Utils.println("Player At Object id: " + objectID
						+ " with Item id: " + itemId);
			break;
		}
	...
	public static void ItemonItem(Client c, int itemUsed, int useWith) {
		c.getOrnaments().ornamenting(itemUsed, useWith);
		...
		if ((itemUsed == 1540 && useWith == 11286)
			...
			if (c.playerLevel[c.playerSmithing] >= 95) {
				...
				c.getPA().addSkillXP(500 * Constants.SMITHING_EXPERIENCE,
						c.playerSmithing);
			} else {
		...
		if (itemUsed == 9142 && useWith == 9190 || itemUsed == 9190
			...
			if (c.playerLevel[c.playerFletching] >= 58) {
				...
				c.getPA().addSkillXP(
						boltsMade * 6 * Constants.FLETCHING_EXPERIENCE,
						c.playerFletching);
			} else {
		...
		if (itemUsed == 9143 && useWith == 9191 || itemUsed == 9191
			...
			if (c.playerLevel[c.playerFletching] >= 63) {
				...
				c.getPA().addSkillXP(
						boltsMade * 7 * Constants.FLETCHING_EXPERIENCE,
						c.playerFletching);
			} else {
		...
		if (itemUsed == 9144 && useWith == 9193 || itemUsed == 9193
			...
			if (c.playerLevel[c.playerFletching] >= 71) {
				...
				c.getPA().addSkillXP(
						boltsMade * 10 * Constants.FLETCHING_EXPERIENCE,
						c.playerFletching);
			} else {
		...
		if (itemUsed == 9144 && useWith == 9194 || itemUsed == 9194
			...
			if (c.playerLevel[c.playerFletching] >= 58) {
				...
				c.getPA().addSkillXP(
						boltsMade * 13 * Constants.FLETCHING_EXPERIENCE,
						c.playerFletching);
			} else {
				c.sendMessage("You need a fletching level of 58 to fletch this item.");
			}
		...
		if (itemUsed == 1601 && useWith == 1755 || itemUsed == 1755
			...
			if (c.playerLevel[c.playerFletching] >= 63) {
				...
				c.getPA().addSkillXP(8 * Constants.FLETCHING_EXPERIENCE,
						c.playerFletching);
			} else {
				c.sendMessage("You need a fletching level of 63 to fletch this item.");
			}
		...
		default:
			if (c.playerRights == 3)
				Utils.println("Player used Item id: " + itemUsed
	...
	public static void ItemonNpc(Client c, int itemId, int npcId, int slot) {
		...
		default:
			if (c.playerRights == 3)
				Utils.println("Player used Item id: " + itemId
