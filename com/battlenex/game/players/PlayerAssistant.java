...
import com.battlenex.utils.Text;
...
public class PlayerAssistant {
	...
	public void changeWhipColor(int changeTo) {
		closeAllWindows();
		...
		case 15443:
			cost = 500;
			if (c.zombiePoints < 500) {
				c.sendMessage("You need 500 Zombie Points in order to change to that color.");
				return;
			}
			break;
		case 15442:
			cost = 350;
			if (c.zombiePoints < 350) {
				c.sendMessage("You need 350 Zombie Points in order to change to that color.");
				return;
			}
			break;
		case 15444:
			cost = 250;
			if (c.zombiePoints < 250) {
				c.sendMessage("You need 250 Zombie Points in order to change to that color.");
				return;
			}
			break;
		case 15441:
			cost = 100;
			if (c.zombiePoints < 100) {
				c.sendMessage("You need 100 Zombie Points in order to change to that color.");
				return;
			}
			break;
		}
		
		if(c.getItems().playerHasItem(4151)) {
			c.zombiePoints -= cost;
			c.getItems().deleteItem(4151, 1);
			c.getItems().addItem(changeTo, 1);
			c.sendMessage("The Abyssal whip in your inventory " + "have been painted the color you selected.");
		} else {
			c.sendMessage("No paintable Abyssal whips could be found in your inventory.");
		}

		/*if (changed) {
			c.sendMessage("All of the Abyssal whips in your inventory " + "have been painted the color you selected.");
			c.getItems().writePlayerItems(3214);
		} else {
			if (found) {
				c.sendMessage("No paintable Abyssal whips could be found in your inventory.");
			} else {
				c.sendMessage("No Abyssal whips could be found in your inventory.");
			}
		}*/
	}
	...
	private static int[] statementIds = { 357, 360, 364, 369 };
	...
	public void showOptions(Client c, String... lines) {
		...
		c.getPA().setInterfaceText("Select an Option", id++);
		for (int i = 0; i < 5; i++) {
			c.getPA().setInterfaceText(i >= lines.length ? "" : lines[i], id++);
		}
	...
	public void setInterfaceText(String s, int id) {
		c.getOutStream().createFrameReservedWord(126);
		c.getOutStream().writeString(s);
		c.getOutStream().writeWordA(id);
		c.getOutStream().endPacketReservedWord();
	}

	public void sendLink(String s) {
		c.getOutStream().createFrameReservedWord(187);
		c.getOutStream().writeString(s);
	}
	
	public void sendDoubleInterface(int mainFrame, int chatBoxInterface) {
		c.getOutStream().createFrame(128);
		c.getOutStream().writeWord(mainFrame);
		c.getOutStream().writeWord(chatBoxInterface);
		sendSidebarInterface(19289, false);
		c.openInterfaceId = mainFrame;
	}
	
	public void sendSidebarInterface(int interfaceId, boolean closeOtherInterfaces) {
		c.getOutStream().createFrame(142);
		c.getOutStream().writeWordReversed(interfaceId);
		c.getOutStream().writeByte(closeOtherInterfaces ? 1 : 0);
	}
	
	public void showRulesInterface() {
		setInterfaceText("1. Do not sell anything on "+Constants.SERVER_NAME+" for RSGP/real life money.", 19001);
		setInterfaceText("2. Do not impersonate staff members.", 19002);
		setInterfaceText("3. Do not advertise other websites/servers.", 19003);
		setInterfaceText("4. Do not abuse bugs, report them immediately.", 19004);
		setInterfaceText("5. Do not scam any user no matter the method.", 19005);
		setInterfaceText("6. Do not use extremely vulgar slurs, including racism.", 19006);
		setInterfaceText("7. Do not ignore staff members.", 19007);
		setInterfaceText("8. Do not encourage others to abuse bugs.", 19008);
		setInterfaceText("9. Do not ask staff members for items.", 19009);
		setInterfaceText("10. Do not ask at all for a staff position.", 19010);
		setInterfaceText("11. Do not disrespect members at all.", 19011);
		setInterfaceText("12. Do not bypass any set filters.", 19012);
		setInterfaceText("13. Do not spam.", 19013);
		setInterfaceText("14. Do not auto-click.", 19014);
		setInterfaceText("15. Do NOT threaten anyone", 19015);
		setInterfaceText("16. Do NOT talk/joke about DDoSing.", 19016);
		setInterfaceText("17. Do NOT joke with people you don't know, they might NOT feel the same way.", 19017);
		setInterfaceText("18. Do NOT argue with any staff members.", 19018);
		setInterfaceText("19. Head Admins have the FINAL decision or punishments etc.", 19019);
		setInterfaceText("20. Do NOT PM Head Administrators in-game for issues - use the Contact Staff forum!", 19020);
		setInterfaceText("21. "+Constants.SERVER_NAME+" is NOT responsible for lost/stolen ingame items due to, but not limited by, server disconnection or internet service provider (ISP) issues. ", 19021);
		setInterfaceText("22. Any activity on your IP-address will be held as YOUR responsibility.", 19022);
		setInterfaceText("23. Making false reports will result in a severe punishment.", 19023);
		setInterfaceText("", 19024);
		setInterfaceText("", 19025);
		setInterfaceText("", 19026);
		setInterfaceText("", 19027);
		setInterfaceText("", 19028);
		setInterfaceText("", 19029);
		setInterfaceText("", 19030);
		sendDoubleInterface(18961, 6179);
	}
	...
	public void setChatOptions(int i, int j, int k) {
		if (c.publicChatMode < 0 || c.publicChatMode > 3) {
			c.publicChatMode = 0;
		}

		if (c.privateChatMode < 0 || c.privateChatMode > 2) {
			c.privateChatMode = 0;
		}

		if (c.tradeMode < 0 || c.tradeMode > 2) {
			c.tradeMode = 0;
		}

		c.getOutStream().createFrame(206);
		c.getOutStream().writeByte(c.publicChatMode);
		c.getOutStream().writeByte(c.privateChatMode);
		c.getOutStream().writeByte(c.tradeMode);
	}
	...
	public void sendPM(long name, int rights, byte[] chatmessage,
		...
		if (c.getOutStream() != null && c != null) {
			...
			c.flushOutStream();
			Utils.textUnpack(chatmessage, messagesize);
			Utils.longToPlayerName(name);
		}
	}
		...
		for (int i = 0; i < c.friends.length; i++) {
			if (c.friends[i] != 0) {
				for (int i2 = 1; i2 < Constants.MAX_PLAYERS; i2++) {
					...
					if (p != null
							&& p.isActive
							&& Utils.playerNameToInt64(p.playerName) == c.friends[i]) {
						Client o = (Client) p;
						if (o != null) {
							if (c.playerRights >= 2
									|| p.privateChat == 0
	...
	public boolean playerIsFriend(long playerName) {
	...
	public void sendFriend(long playerName, int id) {
		c.getOutStream().createFrame(50);
		c.getOutStream().writeQWord(playerName);
	...
	public void sendIgnores() {
		c.getOutStream().createFrameReservedWord(214);
	...
	public void updatePM(int playerId) {
		Client p = (Client) PlayerHandler.getPlayerHandler().players[playerId];
		if (p == null || p.username == null) {
			return;
		}

		long name = Text.nameToLong(p.username);
		...
		if (p.privateChat == 0) {
			...
			for (int i = 0; i < c.friends.length; i++) {
				if (c.friends[i] != 0) {
					if (l == c.friends[i]) {
						if (o.getPA().isInPM(
								Utils.playerNameToInt64(c.playerName))) {
							loadPM(l, world);
							return;
						} else {
							loadPM(l, 0);
		...
		if (c.pitsStatus == 1) {
			...
			movePlayer(
					Constants.DUELING_RESPAWN_X
							+ (Utils.random(Constants.RANDOM_DUELING_RESPAWN)),
					Constants.DUELING_RESPAWN_Y
							+ (Utils.random(Constants.RANDOM_DUELING_RESPAWN)), 0);
	...
	**/
	...
	/**
	 * Levels up using separate class.
	 * @param skill
	 */
	public void levelUp(int skill) {
		 ...
		 */
		case 16:
			sendFrame126("Congratulations, you just advanced an Agility level!",
					4278);
	...
	public void resetBarrows() {
		...
		c.randomCoffin = Utils.random(3) + 1;
	}
	...
	public void writeXPCounterData(int type, int exp) {
		c.getOutStream().createFrame(190);
		c.getOutStream().writeByte(type);
		c.getOutStream().writeDWord(exp);
	}
