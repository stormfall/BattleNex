...
public class SkillMenu {
	...
	public static void openInterface(Player p, int skillType) {
		if (skillType >= SkillMenu.items.length) {
			c.sendMessage("The " + SkillNames.SKILL_NAMES[skillType] + " skill menu is not available right now.");
			return;
		}
		SkillMenu.removeSidebars(c);
		SkillMenu.writeItems(c, skillType);
		SkillMenu.writeText(c, skillType);
		c.getPA().showInterface(SkillMenu.INTERFACE_ID);
	}
