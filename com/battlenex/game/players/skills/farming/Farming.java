package com.battlenex.game.players.skills.farming;
...
public class Farming {
	...
	private void handleFarming(int seedId, int herbId, int exp, int slot) {
		...
		if (c.getItems().playerHasItem(seedId, 1)) {
			...
			c.getPA().addSkillXP(
					SEED_PLANT_EXP[slot] * Constants.FARMING_EXPERIENCE,
					c.playerFarming);
			c.getPA().refreshSkill(c.playerFarming);
			int herbAmount = Utils.random(5) + 3;
			c.farm[0] = herbId;
			c.farm[1] = herbAmount;
			updateHerbPatch();
		}
	...
	public void pickHerb() {
		if (c.farm[0] > 0 && c.farm[1] > 0) {
			if (c.getItems().addItem(c.farm[0], 1)) {
				...
				c.getPA().addSkillXP(getExp() * Constants.FARMING_EXPERIENCE,
						c.playerFarming);
				c.farm[1]--;
				if (c.farm[1] == 0)
					c.farm[0] = -1;
