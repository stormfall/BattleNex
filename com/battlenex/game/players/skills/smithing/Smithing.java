package com.battlenex.game.players.skills.smithing;
	...
	public void smelt(int barType) {
		if (c.smeltAmount > 0) {
			...
			if (hasOres(barType)) {
				...
				c.getPA().addSkillXP(exp * Constants.SMITHING_EXPERIENCE,
						c.playerSmithing);
		...
		if (c.getItems().playerHasItem(toremove, toremove2)) {
			...
			while (maketimes > 0) {
				if (c.getItems().playerHasItem(toremove, toremove2)) {
					...
					c.getPA().addSkillXP(xp * Constants.SMITHING_EXPERIENCE, 13);
