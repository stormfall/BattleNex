package com.battlenex.game.players.skills.thieving;
		...
		for (int j = 0; j < npcThieving.length; j++) {
			if (npcThieving[j][0] == id) {
				if (c.playerLevel[c.playerThieving] >= npcThieving[j][1]) {
					if (Utils.random(c.playerLevel[c.playerThieving] + 2
							- npcThieving[j][1]) != 1) {
						c.getPA().addSkillXP(
								npcThieving[j][2] * Constants.THIEVING_EXPERIENCE,
								c.playerThieving);
	...
	public void stealFromStall(int id, int xp, int level) {
		...
		if (c.playerLevel[c.playerThieving] >= level) {
			if (c.getItems().addItem(id, 1)) {
				...
				c.getPA().addSkillXP(xp * Constants.THIEVING_EXPERIENCE,
						c.playerThieving);
				c.lastThieve = System.currentTimeMillis();
				c.sendMessage("You steal a "
						+ com.battlenex.game.items.Item.getItemName(id) + ".");
			}
		} else {
