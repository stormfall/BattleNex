package com.battlenex.game.players.skills.fishing;
			...
			if (c.playerLevel[c.playerFishing] >= req) {
				if (c.getItems().freeSlots() > 0) {
					if (canFishOther(fishType)) {
						...
						c.getPA().addSkillXP(exp * Constants.FISHING_EXPERIENCE,
								c.playerFishing);
					}
					c.sendMessage("You catch a fish.");
					c.fishTimer = 2 + Utils.random(2);
				}
			} else {
				c.sendMessage("You need a fishing level of " + req
						+ " to fish here.");
