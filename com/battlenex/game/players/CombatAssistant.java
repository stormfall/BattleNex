...
public class CombatAssistant {
	...
	public void attackNpc(int i) {
		...
		if (NPCHandler.npcs[i] != null) {
			...
			if ((c.underAttackBy > 0 || c.underAttackBy2 > 0)
				...
				resetPlayerAttack();
				c.sendMessage("You are already under attack.");
				return;
			}
			if (!goodSlayer(i)) {
				resetPlayerAttack();
			...
			if (c.attackTimer <= 0) {
				...
				if (usingBow && Constants.SOUND == true) {
	...
	public void delayedHit(int i) { // npc hit delay
		if (NPCHandler.npcs[i] != null) {
			...
			if (!c.castingMagic && c.projectileStage > 0) { // range hit damage
				int damage = Utils.random(rangeMaxHit());
				int damage2 = -1;
				if (c.lastWeaponUsed == 11235 || c.bowSpecShot == 1)
					damage2 = Utils.random(rangeMaxHit());
				boolean ignoreDef = false;
				if (Utils.random(5) == 1 && c.lastArrowUsed == 9243) {
					ignoreDef = true;
					NPCHandler.npcs[i].gfx0(758);
				}

				if (Utils.random(NPCHandler.npcs[i].defence) > Utils
				...
				if (Utils.random(4) == 1 && c.lastArrowUsed == 9242
						&& damage > 0) {
				...
				int damage = Utils.random(c.MAGIC_SPELLS[c.oldSpellId][6]);
				if (godSpells()) {
					if (System.currentTimeMillis() - c.godSpellDelay < Constants.GOD_SPELL_CHARGE) {
						damage += Utils.random(10);
					}
				}
				boolean magicFailed = false;
				// c.npcIndex = 0;
				int bonusAttack = getBonusAttack(i);
				if (Utils.random(NPCHandler.npcs[i].defence) > 10
						+ Utils.random(mageAtk()) + bonusAttack) {
					damage = 0;
					magicFailed = true;
				} else if (NPCHandler.npcs[i].npcType == 2881
				...
				if (!magicFailed) {
					...
					case 12911:
					case 12929:
						int heal = Utils.random(damage / 2);
	...
	public void applyNpcMeleeDamage(int i, int damageMask) {
		int damage = Utils.random(calculateMeleeMaxHit());
		...
		if (!fullVeracsEffect) {
			if (Utils.random(NPCHandler.npcs[i].defence) > 10 + Utils
			...
			if (c.attackTimer <= 0) {
				...
				if (!usingBow && !c.usingMagic && !usingOtherRangeWeapons) { // melee
					...
					c.hitDelay = getHitDelay(c.getItems()
							.getItemName(c.playerEquipment[c.playerWeapon])
							.toLowerCase());
					c.delayedDamage = Utils.random(calculateMeleeMaxHit());
					c.projectileStage = 0;
					c.oldPlayerIndex = i;
				}
				...
				if (c.usingMagic) { // magic hit delay
					...
					if (Utils.random(o.getCombat().mageDef()) > Utils
							.random(mageAtk())) {
						c.magicFailed = true;
					} else {
						c.magicFailed = false;
		...
		if (PlayerHandler.players[i] != null) {
			...
			if (!c.castingMagic && c.projectileStage > 0) { // range hit damage
				...
				if (c.lastWeaponUsed == 11235 || c.bowSpecShot == 1)
					damage2 = Utils.random(rangeMaxHit());
				boolean ignoreDef = false;
				if (Utils.random(4) == 1 && c.lastArrowUsed == 9243) {
					ignoreDef = true;
					o.gfx0(758);
				}
				if (Utils.random(10 + o.getCombat().calculateRangeDefence()) > Utils
						.random(10 + calculateRangeAttack()) && !ignoreDef) {
					damage = 0;
				}
				if (Utils.random(4) == 1 && c.lastArrowUsed == 9242
	...
	public void appendMultiBarrage(int playerId, boolean splashed) {
		if (PlayerHandler.players[playerId] != null) {
			...
			if (checkMultiBarrageReqs(playerId)) {
				c.barrageCount++;
				if (Utils.random(mageAtk()) > Utils.random(mageDef())
						&& !c.magicFailed) {
	...
	public void applyPlayerMeleeDamage(int i, int damageMask) {
		...
		if (Utils.random(o.getCombat().calculateMeleeDefence()) > Utils
				.random(calculateMeleeAttack()) && !veracsEffect) {
			damage = 0;
			c.bonusAttack = 0;
		} else if (c.playerEquipment[c.playerWeapon] == 5698
				&& o.poisonDamage <= 0 && Utils.random(3) == 1) {
		...
		case 3204: // d hally
			...
			if (PlayerHandler.players[i] != null && c.playerIndex > 0) {
				if (!c.goodDistance(c.getX(), c.getY(),
					...
					c.doubleHit = true;
					c.delayedDamage2 = Utils.random(calculateMeleeMaxHit());
				}
			}
			break;
			...
			if (c.goodDistance(c.getX(), c.getY(), o.getX(), o.getY(),
				...
				if (checkReqs()) {
					if (checkSpecAmount(4153)) {
						boolean hit = Utils.random(calculateMeleeAttack()) > Utils
								.random(o.getCombat().calculateMeleeDefence());
						int damage = 0;
						if (hit)
							damage = Utils.random(calculateMeleeMaxHit());
						if (o.prayerActive[18]
								&& System.currentTimeMillis()
