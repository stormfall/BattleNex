...
public class TradeAndDuel {
	...
	public void requestTrade(int id) {
		try {
			...
			Utils.println("Error requesting trade.");
		}
	}
			...
			if (item.id > 0) {
				if (item.amount >= 1000 && item.amount < 1000000) {
					...
					SendAmount = "@gre@" + (item.amount / 1000000)
							+ " million @whi@(" + Utils.format(item.amount)
							+ ")";
				} else {
					SendAmount = "" + Utils.format(item.amount);
				}
	...
	public void requestDuel(int id) {
		try {
			...
			Utils.println("Error requesting duel.");
		}
	}
	...
	public void startDuel() {
		...
		if (c.duelRule[8]) {
			if (c.duelRule[1]) {
				...
				c.getPA().movePlayer(3366 + Utils.random(12),
						3246 + Utils.random(6), 0);
			}
		} else {
			if (c.duelRule[1]) {
				c.getPA().movePlayer(c.duelTeleX, c.duelTeleY, 0);
			} else {
				c.getPA().movePlayer(3335 + Utils.random(12),
						3246 + Utils.random(6), 0);
			}
		}
	...
	public void selectRule(int i) { // rules
		...
		if (c.duelRule[8]) {
			if (c.duelRule[1]) {
				c.duelTeleX = 3366 + Utils.random(12);
				o.duelTeleX = c.duelTeleX - 1;
				c.duelTeleY = 3246 + Utils.random(6);
				o.duelTeleY = c.duelTeleY;
			}
		} else {
			if (c.duelRule[1]) {
				c.duelTeleX = 3335 + Utils.random(12);
				o.duelTeleX = c.duelTeleX - 1;
				c.duelTeleY = 3246 + Utils.random(6);
				o.duelTeleY = c.duelTeleY;
			}
