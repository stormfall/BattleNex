package com.battlenex.game.shops;
...
import com.battlenex.game.players.skills.SkillCapes;
...
public class Shop {
	...
	public boolean shopSellsItem(int itemID) {
		for (int i = 0; i < ShopManager.ShopItems.length; i++) {
			if (itemID == (ShopManager.ShopItems[c.myShopId][i] - 1)) {
				return true;
			}
		}
		return false;
	...
	public void openShop(int ShopID) {
		if (c.inTrade) {
			c.sendMessage("You can't shop while trading!");
			return;
		}
		if (c.inDuel) {
			c.sendMessage("You can't shop while dueling!");
			return;
		}
		...
		c.getPA().sendFrame126(ShopManager.ShopName[ShopID], 3901);
	}
	...
	public void resetShop(int ShopID) {
		synchronized (c) {
			...
			for (int i = 0; i < ShopManager.MaxShopItems; i++) {
				if (ShopManager.ShopItems[ShopID][i] > 0) {
					TotalItems++;
				}
			}
			if (TotalItems > ShopManager.MaxShopItems) {
				TotalItems = ShopManager.MaxShopItems;
			}
			...
			for (int i = 0; i < ShopManager.ShopItems.length; i++) {
				if (ShopManager.ShopItems[ShopID][i] > 0
						|| i <= ShopManager.ShopItemsStandard[ShopID]) {
					if (ShopManager.ShopItemsN[ShopID][i] > 254) {
						c.getOutStream().writeByte(255);
						c.getOutStream().writeDWord_v2(
								ShopManager.ShopItemsN[ShopID][i]);
					} else {
						c.getOutStream().writeByte(
								ShopManager.ShopItemsN[ShopID][i]);
					}
					if (ShopManager.ShopItems[ShopID][i] > Constants.ITEM_LIMIT
							|| ShopManager.ShopItems[ShopID][i] < 0) {
						ShopManager.ShopItems[ShopID][i] = Constants.ITEM_LIMIT;
					}
					c.getOutStream().writeWordBigEndianA(
							ShopManager.ShopItems[ShopID][i]);
					TotalCount++;
				}
	...
	public double getItemShopValue(int ItemID, int Type, int fromSlot) {
		...
		if (ShopManager.ShopBModifier[c.myShopId] == 1) {
			TotPrice *= 1;
			TotPrice *= 1;
			if (Type == 1) {
				TotPrice *= 1;
	...
	public void sellToShopPrice(int removeId, int removeSlot) {
		...
		boolean IsIn = false;
		if (ShopManager.ShopSModifier[c.myShopId] > 1) {
			for (int j = 0; j <= ShopManager.ShopItemsStandard[c.myShopId]; j++) {
				if (removeId == (ShopManager.ShopItems[c.myShopId][j] - 1)) {
					IsIn = true;
					break;
				}
			}
	...
	public boolean sellItem(int itemID, int fromSlot, int amount) {
		...
		if (amount > 0 && itemID == (c.playerItems[fromSlot] - 1)) {
			if (ShopManager.ShopSModifier[c.myShopId] > 1) {
				boolean IsIn = false;
				for (int i = 0; i <= ShopManager.ShopItemsStandard[c.myShopId]; i++) {
					if (itemID == (ShopManager.ShopItems[c.myShopId][i] - 1)) {
						IsIn = true;
						break;
					}
	...
	public boolean addShopItem(int itemID, int amount) {
		...
		for (int i = 0; i < ShopManager.ShopItems.length; i++) {
			if ((ShopManager.ShopItems[c.myShopId][i] - 1) == itemID) {
				ShopManager.ShopItemsN[c.myShopId][i] += amount;
				Added = true;
			}
		}
		if (Added == false) {
			for (int i = 0; i < ShopManager.ShopItems.length; i++) {
				if (ShopManager.ShopItems[c.myShopId][i] == 0) {
					ShopManager.ShopItems[c.myShopId][i] = (itemID + 1);
					ShopManager.ShopItemsN[c.myShopId][i] = amount;
					ShopManager.ShopItemsDelay[c.myShopId][i] = 0;
					break;
				}
			}
		}
	...
	public boolean buyItem(int itemID, int fromSlot, int amount) {
		...
		if (amount > 0) {
			if (amount > ShopManager.ShopItemsN[c.myShopId][fromSlot]) {
				amount = ShopManager.ShopItemsN[c.myShopId][fromSlot];
			}
			int TotPrice2 = 0;
			int Slot = 0;
			int Slot1 = 0;
				...
				if (c.myShopId != 29 || c.myShopId != 30 || c.myShopId != 31) {
					if (c.playerItemsN[Slot] >= TotPrice2) {
						if (c.getItems().freeSlots() > 0) {
							...
							ShopManager.ShopItemsN[c.myShopId][fromSlot] -= 1;
							ShopManager.ShopItemsDelay[c.myShopId][fromSlot] = 0;
							if ((fromSlot + 1) > ShopManager.ShopItemsStandard[c.myShopId]) {
								ShopManager.ShopItems[c.myShopId][fromSlot] = 0;
							}
						} else {
							c.sendMessage("You don't have enough space in your inventory.");
							break;
	...
	public int[] skillCapes = SkillCapes.SKILL_CAPE_IDS;
			...
			if (skillCapes[j] == item || skillCapes[j] + 1 == item) {
				if (c.getItems().freeSlots() > 1) {
					if (c.getItems().playerHasItem(995, 99000)) {
						...
						c.sendMessage("You need 99,000gp to buy this item.");
					}
				} else {
