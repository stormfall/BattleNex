...
public class CastleWars {
	...
	public void toWaitingRoom(Client c, int team) {
		if (team == 1) {
			c.getPA().movePlayer(2377 + Utils.random(10), 9485 + Utils.random(5),
					0);
		} else if (team == 2) {
			c.getPA().movePlayer(2417 + Utils.random(10), 9485 + Utils.random(7),
					0);
		}
		c.inCwWait = true;
	}
	...
	public void leaveWaitingRoom(Client c) {
		c.inCwWait = false;
		c.getPA().movePlayer(2439 + Utils.random(4), 3085 + Utils.random(5), 0);
	...
	public void startGame() {
		...
		for (int player : saradominWait) {
			if (PlayerHandler.players[player] != null) {
				...
				c.getPA().movePlayer(2426 + Utils.random(3),
						3076 - Utils.random(3), 1);
		...
		for (int player : zamorakWait) {
			if (PlayerHandler.players[player] != null) {
				...
				Client c = (Client) PlayerHandler.players[player];
				c.getPA().walkableInterface(-1);
				c.getPA().movePlayer(2373 + Utils.random(3),
						3131 - Utils.random(3), 1);
				zamorak.add(player);
				c.inCwWait = false;
				c.inCwGame = true;
			}
